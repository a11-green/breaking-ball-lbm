...
2. **V&V-2（乱流モデル検証）**：一様流中の滑面球・円柱の \(C_D\)-\(Re\) 曲線（既知のドラッグクライシスを含む）を再現できることを確認。
   ...
   **残る容疑者は格子である。** 打ち切り誤差と演算子自身の数値散逸は格子単位で固定量なので、ReとNを固定してνを下げれば νに対する比率が上がる——これは解像度で消え、壁条件では消えないτ依存性である。`--wall` の第4の梯子（τ×N/D）がこれを測る。すなわち、面を開けたあとに残った誤差は幅・後流長・解像度の3つの解像度側の量であり、境界条件側には残っていない。
