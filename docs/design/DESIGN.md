...
**局所グリッドリファインメント（2レベル・4倍細分化）：**

| 粗格子 N/D | 細分化領域 | 細格子 N/D | 細 Δx (mm) | 縫い目/Δx | VRAM | 時間 |
|---|---|---|---|---|---|---|
| 32 | 1.5 D | 128 | 0.584 | 1.35 | 2.49 G | 0.8 h |
| **40** | **1.5 D** | **160** | **0.468** | **1.69** | **4.86 G** | **2.0 h** |
| 32 | 2.0 D | 128 | 0.584 | 1.35 | 3.50 G | 1.5 h |

（細格子は音響スケーリングで1粗ステップあたり4回進めるものとして計上。）

**8D領域と意味のある縫い目解像度を両立させる方法はリファインメントしかない。** 太字の構成は同じ8D領域・同程度のVRAMで、一様格子の0.42セルに対し1.69セルを与える。したがって**局所グリッドリファインメントは最適化項目ではなく要件である**（§9のP4に位置付けを移動）。
...
**未実装：** 細格子内にボールを置くこと（壁・再カット・力のリダクションを細レベルへ）、密結合ループとの接続、GPU移植。
