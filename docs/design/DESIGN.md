...
2. **V&V-2（乱流モデル検証）**：一様流中の滑面球・円柱の \(C_D\)-\(Re\) 曲線（既知のドラッグクライシスを含む）を再現できることを確認。
   ...
   **τ仮説は実測で否定された。** 当初、低Re行の比が小さいのは τ が0.5に張り付くことによる補間バウンスバックの実効壁位置のずれだと考えた（\(\Lambda=(\tau-1/2)^2\) は本番域で \(10^{-9}\)、TRTで厳密な3/16より9桁小さい）。しかしRe=100を固定してτを0.505→0.6と振っても比は0.46–0.49で動かず、N/Dを20→40としても動かない。**離散化誤差は離散化を動かせば動く。動かないのだから離散化誤差ではない。**
