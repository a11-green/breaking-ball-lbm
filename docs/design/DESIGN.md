...
| 壁面バウンスバック | 未実装 | 球表面（補間バウンスバック）、自転速度条件 |
| 力・トルクのリダクション | 未実装 | 運動量交換法の並列リダクション |
...
**【実測で更新】** RTX 3060 Ti上で実測した結果（`scripts/benchmark_gpu.jl`、AAパターン＋融合カーネル、n=64/128/192で頭打ちを確認）：

| 精度 | 衝突演算子 | 実測 MLUPS | 理論上限に対する割合 |
|---|---|---|---|
| FP32 | 中心モーメント | **568** | 27% |
| FP32 | BGK | 524 | 25% |
| FP64 | 中心モーメント | 179 | 17% |
...
ブロックサイズは64〜128が最良（256で-4%、512で-15%）。**当初の見積り（150〜250 MLUPS）は保守的すぎ、FP32では実測が約2.3倍速い。** これにより1条件あたりの計算時間は次のように改訂される：

\[
\frac{3.28\times10^{7}\ \text{点} \times 3.7\times10^{5}\ \text{ステップ}}{5.68\times10^{8}\ \text{更新/秒}} \approx 2.1\times10^{4}\ \text{秒} \approx \textbf{6時間/条件}
\]

（当初見積り15〜20時間に対して約1/3。）VRAMの実測上限はFP32で約4600万点、すなわち一辺358格子の立方体まで。§6.5の推奨構成（320³, 約3300万点）は余裕を持って収まる。

**FP64は使わない**：このカードのFP64演算は1/64レートであり、実測でもFP32の約1/3の速度しか出ない。本番はFP32とする（Taylor-Green渦での粘性再現はFP32でも誤差-0.06%で、精度上の問題はない）。

なお実測はまだ理論上限の27%に留まっており、スレッドあたり27個の分布関数を保持するバッファがレジスタに収まらずローカルメモリへ退避している可能性が高い（ブロックサイズを上げると性能が落ちることもこれを支持する）。改善余地はあるが、6時間/条件は当初計画に対して十分な水準である。

**この見積りは未較正である。** メモリ帯域（RTX 3060 Ti: 約448 GB/s）から素朴に上限を求めると、1ノード更新あたり27回の読み出し＋27回の書き込み＝FP32で216バイトなので、**約2,070 MLUPS** が理論上限となる。実装が上限の30〜60%に達するなら600〜1,200 MLUPS であり、上記の見積り（150〜250 MLUPS）は数倍保守的すぎる可能性がある。逆にAAパターンの奇数ステップは隣接ノードへの非連続アクセスであり、スレッドローカルバッファがローカルメモリに退避されれば実効帯域は落ちる。**どちらに転ぶかは実測でしか分からない**ため、`scripts/benchmark_gpu.jl` が達成MLUPSと理論上限に対する割合を出力するようにしてある。
...
| P2 ✅（周期境界コア） | CUDA.jlによる単一GPU実装。AAパターン＋衝突の融合カーネルをRTX 3060 Ti上で実行し、CPU参照実装との一致（FP64で1.7e-15、FP32で8.0e-7）とTaylor-Green渦からの粘性再現（誤差-0.06%）を確認。MLUPS実測により§6.6を更新。**壁面境界と力のリダクションのGPU化は次段階** |
