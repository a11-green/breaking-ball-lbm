...
- **流入・流出境界（優先度が上がった）**：V&V-2高Re側（§8）の結果、周期箱では抗力係数の基準速度そのものが後流の巻き戻りで曖昧になり、その曖昧さが測ろうとしている差より大きくなりうることが実測で分かった。同じ箱を本番の密結合ループも使っている（§4.4）ので、これは検証だけの問題ではない。上流面に一様流入、下流面に対流流出（および必要なら緩衝層）を実装し、平均流コントローラを置き換える。**実装すべきかどうかではなく、いつ実装するかの問題になった**——これが入るまで、抗力の絶対値に対する主張はすべて上下限付きでしか書けない。
