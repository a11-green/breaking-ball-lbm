...
| P2 (進行中) | CUDA.jlによる単一GPU実装。周期境界のコア（AAパターン＋衝突の融合カーネル）と自己検証付きベンチマークを実装済み（`ext/`, `scripts/benchmark_gpu.jl`）。**RTX 3060 Ti上での実行とMLUPS実測は未実施**（開発環境にGPUがないため）。壁面境界のGPU化は次段階 |
