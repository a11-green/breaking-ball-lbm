...
| P4 🚧 | ボール追従座標系＋6DOF密結合ループの実装（§4.4, §5）。**軌道側（四元数姿勢・RK4による6DOF・非慣性系の体積力・格子単位系・変化量メトリクス）は実装・検証済み**（`src/trajectory/`, `test/test_trajectory.jl`）。**密結合ループ本体もCPUで実装・検証済み**（`src/trajectory/coupling.jl`, `test/test_coupling.jl`）。結合ループのGPU移植も完了（`gpu_flow`）。残：流入・流出境界、局所グリッドリファインメント、縫い目回転に伴う壁ジオメトリの再構築 |
