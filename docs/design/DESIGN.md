...
**本番ドライバでの使い方。** `scripts/run_pitch.jl --open --upstream D --downstream D`。開放時は `--domain` は横幅だけを決め、箱は立方体ではなく管になる（流れ方向は助走＋後流）。ボールは入口から `upstream` 直径の位置に置かれ、スナップショットの切り出しもボール中心に追従する。リファインメントとの併用は不可（上記の理由で、起動時に拒否する）。疎通確認（CPU, 60×48×48, 助走2D）で中心線は入口端0.0500（指定値そのもの）→よどみ点0.0069→後流0.0097から0.039へ回復、と読める。
...
- 流れ場：VTK（ParaView対応）、渦度・Q値等の可視化フィールドを出力。**実装済み**（`src/postprocess/vtk.jl`、依存追加なし——レガシーVTKバイナリはテキストヘッダ＋ビッグエンディアンのfloat列で、自前で50行。`WriteVTK.jl` を足すと実行環境側にもインストールが必要になる）。`scripts/run_pitch.jl --snapshot N` で N サブサイクルごとに書く。出力は密度・速度・速度の大きさ・渦度・渦度の大きさ・Q値・固体マスク。
  ...
  **サイズが設計の全部である。** 本番格子は320³で、単精度5場なら1スナップショットが1GB——数サブサイクルごとに書けば板に着く前にディスクが尽きる。したがって `--snapshot-crop`（ボール周りの半幅、直径単位。追従座標系ではボールは箱の中央に留まる）と `--snapshot-stride`（間引き。**微分を取った後に**間引くので、渦度は粗い格子のものではなく本来の格子のものになる）。リファインド経路は未対応で、その旨を印字して飛ばす（細パッチと粗格子が別グリッドのため）。
