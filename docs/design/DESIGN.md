...
本番（40点/D）の流体計算そのものは320³で0.7時間（§6.6）なので、**CPUでの再カットはその5倍以上**になる。**したがって再カットはGPUへ移す必要がある**。作業量自体は大きくない（境界節点数千 × 固体リンク数本 × 二分法20回）——単に逐次で、置き場所が間違っているだけで、ホストを必要とする部分は何もない。
