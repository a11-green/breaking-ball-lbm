...
**測定（CPU、N/D=6、横2.5D、Re=20の疎通確認）。** 球の1.5D上流の前面円板で測った流速は、周期箱で箱平均の **0.421**、入口を付けると入口指定値の **0.972**。これが機構そのものである。本番相当の解像度での値は `scripts/validate_sphere_highre.jl --open` が出す。
