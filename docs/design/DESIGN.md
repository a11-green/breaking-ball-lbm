...
物体に沿った形（球殻など）が有利に見えるのは節点数だけである：同じ幅の球は立方体の **52%** の体積しかない。しかしその節約は**格納形式を密な直方体配列からリストに変えたときにだけ実体化する**。密な箱に球を入れてマスクするなら節点数は1つも減らない。リストにすると、隣接節点がストライドの加算ではなく索引の引き直しになる——AAパターンは1節点27方向を読むので、間接参照は27回の余分なロードとコアレスの喪失を意味する。**球が払えるのは間接参照が2倍未満の場合だけ**で、その比は `scripts/benchmark_gpu.jl` が実測する（`gpu_gather_bandwidth`、シャッフルした置換で測る——整列した索引ではキャッシュを測ってしまう）。
